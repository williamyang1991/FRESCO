...
int main(int argc, char **argv)
{
    ...
    while (argi < argc && !fail)
    {
      ...
      else if (tryToParseFloatArg(args, &argi, "-weight", &weight, &fail))
      {
        if (precedingStyleOrGuideWeight != 0)
        {
          if (weight >= 0)
          {
            *precedingStyleOrGuideWeight = weight;
          }
          else
          {
            printf("error: weights must be non-negaitve!\n");
            return 1;
          }
        }
        else
        {
          printf("error: at least one -style or -guide option must precede the -weight option!\n");
          return 1;
        }
        argi++;
      }
  ...
  for (int xy = 0; xy < sourceWidth * sourceHeight; xy++)
  {
    ...
    {
      sourceStyle[xy * numStyleChannelsTotal + 0] = sourceStyleData[xy * 4 + 0];
    }
    if (numStyleChannelsTotal == 2)
    {
      sourceStyle[xy * numStyleChannelsTotal + 1] = sourceStyleData[xy * 4 + 3];
    }
    else if (numStyleChannelsTotal > 1)
    {
      sourceStyle[xy * numStyleChannelsTotal + 1] = sourceStyleData[xy * 4 + 1];
    }
    if (numStyleChannelsTotal > 2)
    {
      sourceStyle[xy * numStyleChannelsTotal + 2] = sourceStyleData[xy * 4 + 2];
    }
    if (numStyleChannelsTotal > 3)
    {
      sourceStyle[xy * numStyleChannelsTotal + 3] = sourceStyleData[xy * 4 + 3];
    }
  }
  ...
  for (int i = 0; i < numGuides; i++)
  {
    ...
    if (guide.sourceWidth != sourceWidth || guide.sourceHeight != sourceHeight)
    {
      printf("error: source guide '%s' doesn't match the resolution of '%s'\n", guide.sourceFileName.c_str(), styleFileName.c_str());
      return 1;
    }
    if (i > 0 && (guide.targetWidth != targetWidth || guide.targetHeight != targetHeight))
    {
      printf("error: target guide '%s' doesn't match the resolution of '%s'\n", guide.targetFileName.c_str(), guides[0].targetFileName.c_str());
      return 1;
    }
    else if (i == 0)
    {
      targetWidth = guide.targetWidth;
      targetHeight = guide.targetHeight;
    }
  ...
  for (int xy = 0; xy < sourceWidth * sourceHeight; xy++)
  {
    ...
    for (int i = 0; i < numGuides; i++)
    {
      ...
      if (numChannels > 0)
      {
        sourceGuides[xy * numGuideChannelsTotal + c + 0] = guides[i].sourceData[xy * 4 + 0];
      }
      if (numChannels == 2)
      {
        sourceGuides[xy * numGuideChannelsTotal + c + 1] = guides[i].sourceData[xy * 4 + 3];
      }
      else if (numChannels > 1)
      {
        sourceGuides[xy * numGuideChannelsTotal + c + 1] = guides[i].sourceData[xy * 4 + 1];
      }
      if (numChannels > 2)
      {
        sourceGuides[xy * numGuideChannelsTotal + c + 2] = guides[i].sourceData[xy * 4 + 2];
      }
      if (numChannels > 3)
      {
        sourceGuides[xy * numGuideChannelsTotal + c + 3] = guides[i].sourceData[xy * 4 + 3];
      }
  ...
  for (int xy = 0; xy < targetWidth * targetHeight; xy++)
  {
    ...
    for (int i = 0; i < numGuides; i++)
    {
      ...
      if (numChannels > 0)
      {
        targetGuides[xy * numGuideChannelsTotal + c + 0] = guides[i].targetData[xy * 4 + 0];
      }
      if (numChannels == 2)
      {
        targetGuides[xy * numGuideChannelsTotal + c + 1] = guides[i].targetData[xy * 4 + 3];
      }
      else if (numChannels > 1)
      {
        targetGuides[xy * numGuideChannelsTotal + c + 1] = guides[i].targetData[xy * 4 + 1];
      }
      if (numChannels > 2)
      {
        targetGuides[xy * numGuideChannelsTotal + c + 2] = guides[i].targetData[xy * 4 + 2];
      }
      if (numChannels > 3)
      {
        targetGuides[xy * numGuideChannelsTotal + c + 3] = guides[i].targetData[xy * 4 + 3];
      }
  ...
  {
    styleWeights[i] = styleWeight / float(numStyleChannelsTotal);
  }

  for (int i = 0; i < numGuides; i++)
  {
    if (guides[i].weight < 0)
    {
      guides[i].weight = 1.0f / float(numGuides);
    }
  }
